...
async def check_waitlist_queue_enforcement(customer_id: str, room_type: str, manager_override: bool = False, manager_password: str = None, current_user = None) -> str:
    ...
    if manager_override:
        ...
        manager_override_password = os.environ.get('MANAGER_OVERRIDE_PASSWORD')
...
async def complete_checkin(completion_data: dict, current_user: User = Depends(get_current_user),
    ...
    pending_checkin = await location_db.pending_check_ins.find_one({
...
async def prepare_waitlist_upgrade(entry_id: str, upgrade_data: dict, current_user: User = Depends(get_current_user)):
    ...
    new_room_number = upgrade_data.get("room_number")
    new_room_type = upgrade_data.get("room_type", waitlist_entry["desired_room_type"])
    ...
    # Get current check-in if customer is already checked in
    current_checkin = await db.check_ins.find_one({
        "customer_id": customer_id,
        "check_out_time": None
    })
...
async def get_membership_form_qr():
    ...
    try:
        ...
        frontend_url = os.environ.get('FRONTEND_URL', 'https://flexspa-manager-1.preview.emergentagent.com')
