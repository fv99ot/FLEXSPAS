            ...
            <Card className="dashboard-card" style={{ background: 'rgba(15, 23, 42, 0.8)', border: '1px solid rgba(30, 58, 138, 0.3)', color: '#ffffff' }}>
              <CardHeader>
                <CardTitle className="flex items-center justify-between text-white">
                  ...
                  <Button onClick={fetchTransactions} className="flex-button">
                    <Clock className="h-4 w-4 mr-2" />
                    Refresh
                  </Button>
                </CardTitle>
                <CardDescription className="text-gray-300">
                  View and manage transaction records - {transactions.length} transactions loaded
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-3 max-h-96 overflow-y-auto">
                  {transactions.map((transaction) => (
                    <div key={transaction.id} className="flex items-center justify-between p-3 border border-white/20 rounded-lg bg-white/5">
                      <div className="flex-1">
                        <div className="flex items-center justify-between mb-2">
                          <h4 className="font-medium text-white">{transaction.customer_name}</h4>
                          <div className="flex items-center space-x-2">
                            <Badge variant={transaction.is_refund ? "destructive" : "default"} className="text-xs">
                              {transaction.transaction_type}
                            </Badge>
                            <span className={`font-semibold ${transaction.is_refund ? 'text-red-400' : 'text-green-400'}`}>
                              {transaction.is_refund ? '-' : ''}${Math.abs(transaction.total_amount).toFixed(2)}
                            </span>
                          </div>
                        </div>
                        <div className="flex items-center justify-between text-sm text-gray-300">
                          ...
                          <div className="flex items-center space-x-4">
                            ...
                            {transaction.discount_name && (
                              <span className="text-yellow-400">Discount: {transaction.discount_name}</span>
                            )}
                            {transaction.items && transaction.items.length > 0 && (
                              <span className="text-blue-400">{transaction.items.length} item(s)</span>
                            )}
                          </div>
                        </div>
                      </div>
                      <div className="flex space-x-2 ml-4">
                        {!transaction.is_refund && user?.role === 'manager' && (
                          <Button
                            size="sm"
                            variant="outline"
                            className="border-red-500 text-red-500 hover:bg-red-500 hover:text-white"
                  ...
                  {transactions.length === 0 && (
                    <div className="text-center py-8">
                      <p className="text-gray-400">No transactions found. Transactions will appear here after payments are processed.</p>
                    </div>
                  )}
                </div>
              </CardContent>
